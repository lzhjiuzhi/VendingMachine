...
public class AddRecordModel extends AbstractTableModel{
	...
	private Vector<Vector<String>> rowData = null;	//行のデータ
	 ...
	 public AddRecordModel(JDialog jd) {
			...
			columnNames.add("補充前の金額");
			columnNames.add("補充した金額");
			columnNames.add("補充後の金額");
			...
			if(records!=null){
				for(AddRecords record : records){
					...
					hang.add(record.getBeforeAdd()+"円");
					hang.add(record.getAddAmount()+"円");
					hang.add(record.getAfterAdd()+"円");
