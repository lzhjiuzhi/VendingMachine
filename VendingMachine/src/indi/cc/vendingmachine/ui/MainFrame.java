    	...
    	Vector<Drink> drinks = manageHelper.getAllDrink();	//获得所有的饮料
    	 for(int i=0;i<drinks.size();i++){
         	...
         	if(drinks.get(i).getQuantity()==0){
         		...
         		s = drink.getDrinkName()+"|価額："+drink.getPrice()+"円|数量："+drink.getQuantity();
         		jb = new JButton(s,new ImageIcon(drink.getDrinkImg()));
         	}
    ...
    private void computing(){
		...
		if(price==0){
			...
			if(oncePay!=0){
				...
				JOptionPane.showMessageDialog(thisJframe, "お返し金額をお受け取りください！！");
				jTextField1.setText("");
			}else{
			...
			case 10:
					need = need - 10;
					if(need!=0){
						JOptionPane.showMessageDialog(thisJframe, "必要な金額："+need+"円");
					}
					pay += oncePay;	//
					break;
			case 50:
...
//					break;
			...
			if(repayment!=0){
				JOptionPane.showMessageDialog(thisJframe, "お返し金額："+repayment+"円");
			}
			if(price!=0 && need==0){//付款成功
				JOptionPane.showMessageDialog(thisJframe, "購入完成！商品が出るまでお待ちください！！");
				//对应的饮料数量需要减少!!
				//顾客操作的帮助类实现饮料数量的减少
