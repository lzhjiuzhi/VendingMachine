       ...
       //注册事件
        jButton1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(username.equals("")){
					JOptionPane.showMessageDialog(jf, "ユーザー名は空にできません！！");
					return ;
				}else if(password.equals("")){
					JOptionPane.showMessageDialog(jf, "パスワードは空にできません！！");
					return ;
				}else{
					Administrator admin = new Administrator(username,password);
					AdminManageHelper manageHelper = new AdminManageHelper();
					if(manageHelper.Login(admin)){
						JOptionPane.showMessageDialog(jf, "登録できました！！");
						AdminChoose adminChoose = new AdminChoose(admin);
						jf.dispose();
						return ;
					}else{
						JOptionPane.showMessageDialog(jf, "ユーザー名またはパスワードは間違っています！！");
						jTextField1.setText("");
